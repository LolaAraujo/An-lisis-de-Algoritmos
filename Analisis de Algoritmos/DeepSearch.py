...
class deepSearch(object):
    def run(self,mapa,start,end):
        startNode=MapaNode(start[::-1])  #-1y -1 en pila #-1y1 en cola en mapa de profundidad, para mapa de expanción 1,-1
        ...
        while(len(fila)!=0 ):
            currentNode=fila.pop(0)    # con 0 para el desplazamiento y sin 0 para profundidad
...
#                    Agregamos el nodo a la fila y de padre queda el que acabamos de sacar
                    adjacentNode=MapaNode(newPosition,currentNode)
                    fila.append(adjacentNode)
...
start= [43,43]   #segundo mapa, 43,43, primer mapa 3,28, original 10,3
#punto final en formato columa,fila (x,y)
goal= [12,10]    #segundo mapa 12,10, primer mapa 24,2, original 5,3
